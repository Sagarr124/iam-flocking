    ...
    def update(self, metric_override=None, mode="topological"):
        ...
        if mode == "topological":
            ...
            if Config.BLIND_SPOT_ANGLE_DEG > 0:
                ...
                neighbor_dir = dx / dx_norm

                heading_expanded = heading[:, np.newaxis, :]
                cos_angle = np.sum(heading_expanded * neighbor_dir, axis=2)
            ...
            forces_sep = np.zeros((self.N, 3))
            steering_ali = np.zeros((self.N, 3))
            steering_coh = np.zeros((self.N, 3))
